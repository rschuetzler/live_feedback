defmodule LiveFeedbackWeb.CoreComponents do
  @moduledoc """
  ...
  import LiveFeedbackWeb.Gettext
    ...
    <div class="sm:px-4 sm:px-0">
      <table class="mt-11 w-full">
        <thead class="text-sm leading-6 text-left text-zinc-500">
          <tr>
            <th :for={col <- @col} class="p-0 pb-4 sm:pr-6 font-normal"><%= col[:label] %></th>
        ...
        >
          <tr :for={row <- @rows} id={@row_id && @row_id.(row)} class="group hover:bg-zinc-50">
            ...
            >
              <div class="block py-4 pr-2 sm:pr-6">
            ...
            <td :if={@action != []} class="relative p-0">
              <div class="flex justify-between py-4 text-sm font-medium text-right whitespace-nowrap">
                ...
                <span
                  :for={action <- @action}
                  class="ml-1 font-semibold leading-6 text-zinc-900 hover:text-zinc-700"
                >
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(LiveFeedbackWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(LiveFeedbackWeb.Gettext, "errors", msg, opts)
    end
  end
